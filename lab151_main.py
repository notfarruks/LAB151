...
logger = logging.getLogger("lab151")
...
# ─── App ──────────────────────────────────────────────────────────────────────
app = FastAPI(title="LAB151")
app.add_middleware(TrustedHostMiddleware, allowed_hosts=["*"])
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])
executor = ThreadPoolExecutor(max_workers=4)

# ─── Env ──────────────────────────────────────────────────────────────────────
BOOKING_URL      = os.environ.get("BOOKING_URL", "https://lab151-production.up.railway.app/book")
LANDING_URL      = BOOKING_URL.rsplit("/book", 1)[0] or BOOKING_URL  # base URL for landing page
SALON_PHONE      = os.environ.get("SALON_PHONE", "+994775734545")  # Cosmetology
SALON_PHONE_NAILS = os.environ.get("SALON_PHONE_NAILS", "+994774434545")  # Nails & Hair
CANCEL_CUTOFF_MINUTES = int(os.environ.get("CANCEL_CUTOFF_MINUTES", "60"))  # minutes before appt to allow cancel
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "lab151admin")
...
def is_slot_held(master_id: str, date_str: str, time_str: str) -> bool:
    return redis_client.exists(f"lab151:hold:{master_id}:{date_str}:{time_str}") > 0
...
def reschedule_booking(row: int, new_date: str, new_time: str):
    try:
        sheet = get_sheets().open("LAB151_DB").worksheet("Bookings")
        ...
        async with httpx.AsyncClient() as c:
            r = await c.post(
                ...
                json={
                    ...
                    "from": {"email": SENDER_EMAIL, "name": "LAB151 beauty chargers"},
                    "subject": subject,
        ...
        try:
            ...
            for i, b in enumerate(sheet.get_all_records(), 2):
                if (str(b.get("date")) == td and
                    ...
                    if email:
                        subject = f"LAB151 — Reminder: tomorrow at {b.get('time')}"
                        body = f"""
<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;padding:24px;color:#222">
  <h1 style="font-size:22px;margin-bottom:4px">LAB151</h1>
...
async def api_post_review(req: Request):
    try:
        ...
        # master_id/name can be passed directly (open review with no booking_id)
        req_master_id   = str(body.get("master_id","")).strip()
        req_master_name = str(body.get("master_name","")).strip()[:100]

        if rating < 1 or rating > 5:
            raise HTTPException(400, "rating (1-5) required")

        # Prevent duplicate reviews for the same booking (only when booking_id given)
        if booking_id and any(r["booking_id"] == booking_id for r in REVIEWS):
            return JSONResponse({"ok": False, "error": "already_reviewed"})

        master_id   = req_master_id
        master_name = req_master_name or MASTERS.get(master_id, {}).get("name", "")
        try:
            client = get_sheets()
            db     = client.open("LAB151_DB")
            # If booking_id given, look it up to get master info
            if booking_id:
                sheet    = db.worksheet("Bookings")
...
async def api_book(req: BookingRequest):
    ...
    sms_msg = (
        f"LAB151: Rezervasiyaniz tedsdiqlendi!\n"
...
async def serve_admin_page():
    html_path = Path(__file__).parent / "lab151_admin.html"
    if html_path.exists():
...
async def serve_landing_page():
    html_path = Path(__file__).parent / "lab151_landing.html"
    if html_path.exists():
        return HTMLResponse(html_path.read_text(encoding="utf-8"))
...
async def serve_booking_page():
    html_path = Path(__file__).parent / "lab151_booking.html"
    if html_path.exists():
        return HTMLResponse(html_path.read_text(encoding="utf-8"))
...
GREETER = {
    "az": (
        "👋 *LAB151* salonuna xoş gəlmisiniz!\n\n"
...
def get_wa_session(phone):
    d = redis_client.get(f"lab151:wa:{phone}")
...
def save_wa_session(phone, s):
    redis_client.setex(f"lab151:wa:{phone}", 86400, json.dumps(s))
